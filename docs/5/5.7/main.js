...
result.innerHTML += `\t\`(3 vec a - vec b + 2 vec c, 2 vec a - vec b + vec c) = (${toString(math.multiply(a, 3))} - ${toString(b)} + ${toString(math.multiply(2, c))}) * (${toString(math.multiply(2, a))} - ${toString(b)} + ${toString(c)}) = ${toString(leftSide)} * ${toString(rightSide)} = ${math.multiply(leftSide, rightSide)}\``
